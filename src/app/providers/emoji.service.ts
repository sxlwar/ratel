...
import {
  ...
  EmojiVariation,
  Emoji
} from '../interface/emoji.interfaces';
import { emojis } from '../constant/emoji/emojis';

const COLONS_REGEX = /^(?:\:([^\:]+)\:)(?:\:skin-tone-(\d)\:)?$/;
const SKINS = ['1F3FA', '1F3FB', '1F3FC', '1F3FD', '1F3FE', '1F3FF'];
export const DEFAULT_BACKGROUND_FN = (
  set: string,
  sheetSize: number,
...
export class EmojiService {
  ...
  getStyle(emoji: string): string {
      ...
      return Object.entries(spriteStyle).reduce((pre, [key, value]) => `${pre}; ${key}: ${value}; `, 'margin: 0 5px 0 0');
  }
}
